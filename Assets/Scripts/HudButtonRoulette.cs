...
public class HudButtonRoulette : MonoBehaviour
{
	...
	private void Initialize()
	{
		...
		{
			GameObject gameObject = mainMenuUIObject.transform.Find("Anchor_8_BC/Btn_roulette").gameObject;
			if (gameObject != null)
			{
				m_free_badge = gameObject.transform.Find("badge_spin").gameObject;
				if (m_free_badge != null)
				{
					GameObject gameObject2 = m_free_badge.transform.Find("Lbl_roulette_volume").gameObject;
					if (gameObject2 != null)
					{
						m_free_label = gameObject2.GetComponent<UILabel>();
					}
				}
				m_chao_free_badge = gameObject.transform.Find("badge_p_spin").gameObject;
				if (m_chao_free_badge != null)
				{
					GameObject gameObject3 = m_chao_free_badge.transform.Find("Lbl_roulette_p_volume").gameObject;
					if (gameObject3 != null)
					{
						m_chao_free_label = gameObject3.GetComponent<UILabel>();
					}
				}
				m_egg_obj = gameObject.transform.Find("badge_egg").gameObject;
				m_sale_obj = gameObject.transform.Find("badge_alert").gameObject;
				m_event_obj = gameObject.transform.Find("event_icon").gameObject;
				GameObject gameObject4 = gameObject.transform.Find("img_ad_tex").gameObject;
				if (gameObject4 != null)
				{
