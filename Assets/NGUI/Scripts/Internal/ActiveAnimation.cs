...
public class ActiveAnimation : MonoBehaviour
{
	...
	void Update ()
	{
		float delta = (!mRealTime) ? Time.deltaTime : RealTime.deltaTime;
		if (delta == 0f) return;
	...
	{
		return Play(anim, clipName, playDirection, enableBeforePlay, disableCondition, true);
	}
	...
	static public ActiveAnimation Play (Animation anim, string clipName, Direction playDirection,
		EnableCondition enableBeforePlay, DisableCondition disableCondition, bool flagRealTime)
	{
	...
	{
		return Play(anim, clipName, playDirection, EnableCondition.DoNothing, DisableCondition.DoNotDisable, flagRealTime);
	}

	public static ActiveAnimation Play(Animation anim, Direction playDirection, bool flagRealTime)
	{
		return Play(anim, null, playDirection, EnableCondition.DoNothing, DisableCondition.DoNotDisable, flagRealTime);
	}
