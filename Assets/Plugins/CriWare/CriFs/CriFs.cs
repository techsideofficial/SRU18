...
public class CriFsLoader : IDisposable
{
	 ...
	 * \attention
	 * 本関数を実行しても、ローダのステータスが ::CriFsLoader::Status が Stop に変わるまでは、
...
public class CriFsInstaller : IDisposable
{
	 ...
	 * <param name="installBufferSize">インストールバッファサイズ</param>
	 * \par 説明:
	 * ファイルのコピーを開始します。<br/>
	 * <br/>
	 * 本関数は即時復帰関数です。<br/>
...
public class CriFsBinder : IDisposable
{
	...
	/**
	 * <summary>ファイルの一部分をバインドし、その箇所を仮想的なファイルとして扱えるよう設定します。</summary>
	 * <param name="srcBinder">バインドするファイルにアクセスするためのバインダ</param>
	 * <param name="path">バインドするファイルのパス名</param>
 ...
 * \par 説明:
 * インストール処理の進捗状況の確認や、インストール処理結果を取得するためのモジュールです。<br/>
 * ::CriFsUtility::WebInstall 関数の戻り値として返されます。<br/>
 * <br/>
 * インストール処理が完了すると、isDoneの値がtrueになります。<br/>
 * インストール処理中にエラーが発生した場合、errorにエラー情報が格納されます。<br/>
 * \sa CriFsUtility::WebInstall
 */
public class CriFsWebInstallRequest : CriFsInstallRequest
{
	 ...
	 * 本関数は即時復帰関数です。<br>
	 * 本関数を実行してからインストール処理が停止するまでには、最大20秒程度の時間がかかることがあります。<br>
	 * インストール処理が停止すると、コルーチンとして動作している ::CriFsWebInstallRequest::WaitForDone 関数が復帰します。<br>
	 */
	public override void Stop()
	{
		if (this.installer != null) {
...
public static class CriFsUtility
{
	 ...
	 * 本関数の代わりに ::CriFsUtility::LoadFile を使用し、
	 ...
	 * ロード処理中にエラーが発生した場合、 ::CriFsLoadFileRequest::error にエラー情報が格納されます。<br/>
	 * \par 備考:
	 * 第一引数にバインダを指定する点以外は、 ::CriFsUtility::LoadFile 関数と同じです。<br/>
	 * \sa
	 ...
	 * 本関数の代わりに ::CriFsUtility::LoadAssetBundle を使用し、
	 * データ読み込み元となるバインダを指定する必要があります。<br/>
	 * <br/>
	 * パスにURLを指定することで、ネットワーク経由でファイルをロードすることも可能です。<br/>
	 * \code
	 ...
	 * ロード処理中にエラーが発生した場合、 ::CriFsLoadFileRequest::error にエラー情報が格納されます。<br/>
	 * \par 備考:
	 * 第一引数にバインダを指定する点以外は、 ::CriFsUtility::LoadAssetBundle 関数と同じです。<br/>
	 * \sa
	 ...
	 * \endcode
	 * \attention
	 * インストール先が存在しない場合エラーになるため、事前にフォルダ作成して本APIを呼び出してください。<br/>
	 * \sa
	 * CriFsInstallRequest
	 */
	...
	/**
	 * <summary>ファイルのインストールを開始します。</summary>
	 * <param name="srcPath">インストール元ファイルパス(URL)</param>
	 * <param name="dstPath">インストール先ファイルパス</param>
	 * <param name="doneDeleagate">非同期処理の完了コールバック</param>
	 * <returns>CriFsWebInstallRequest</returns>
	 * \par 説明:
	 * ファイルのインストールを開始します。<br/>
	 * インストール元ファイルパス(URL)をsrcPath、インストール先ファイルパスをdstPathで指定します。<br/>
	 * <br/>
	 * 本関数は即時復帰関数です。<br/>
	 * インストールの完了をチェックするには、 ::CriFsWebInstallRequest::isDone を確認する必要があります。<br/>
	 * インストール処理中にエラーが発生した場合、 ::CriFsWebInstallRequest::error にエラー情報が格納されます。<br/>
	 * \attention 
	 ...
	 * （更新後のCPKファイルに対し意図しないアクセスが発生する可能性があります。）<br/>
	 * \sa
	 ...
	 * <param name="proxyPort">プロキシサーバポート番号</param>
	 * \par 説明：
	...
	/**
	 * <summary>パスセパレータの指定</summary>
	 * <param name="filter">セパレータとして使用する文字の一覧</param>
	 * \par 説明：
	 * CRI File Systemライブラリがセパレータとして解釈する文字を変更します。<br>
	 * デフォルト状態では「,」「\\t」「\\n」の3つの文字がセパレータとして扱われます。<br>
	 * filter には、セパレータとして使用する文字の一覧を格納した文字列を指定します。<br>
	 * 例えば、 "@+-*" を指定した場合、「@」「+」「-」「*」の4種類の文字がセパレータ
	 * として扱われます。<br>
	 * filter に空文字列("")を指定した場合はセパレータが無効になり、 null を指定した
	 : 場合は設定がデフォルトに戻ります。<br>
	 * \attention
	 * プラグイン関数に対して本関数で指定したセパレータを含むパスを渡した場合、内部で
	 * 不正なパスとして処理され、セパレータ以降のパスが無効になります。<br>
	 * アプリケーション中で「,」「\\t」「\\n」を含むパスを扱う場合には、本関数で事前に
	 * セパレータに変更しておく必要があります。<br>
	 */
	public static void SetPathSeparator(string filter)
	{
		criFsUnity_SetPathSeparator(filter);
	}
